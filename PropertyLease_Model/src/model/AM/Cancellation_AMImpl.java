...
//                    
    ...
    //create PM fee direct OC for cancellation
            public String createPmFeeDirectOC(String cnId)
            {
                String flag="null";
                CallableStatement cs=this.getDBTransaction().createCallableStatement("{call auto_create_pkg.CREATE_PM_FEE_D_OC_CN(?,?,?)}",1);
                try {
                    cs.setString(1,cnId);
                    cs.registerOutParameter(2,Types.VARCHAR);
                    cs.registerOutParameter(3,Types.VARCHAR);
    ...
    {
      String flag="null";
      CallableStatement cs=this.getDBTransaction().createCallableStatement("{call NEW_APPROVAL_FLOW_PKG.SUBMIT_FOR_APR_CN(?,?,?,?)}",1);
      try {
          cs.setString(1,cnId);
    ...
    //approve or reject//Respose of approval
    public String responseCnForAppr(String cnId,String reason,String apr_rej)
    {
        String flag="null";
        CallableStatement cs=this.getDBTransaction().createCallableStatement("{call NEW_APPROVAL_FLOW_PKG.RESPONSE_FOR_APR_CN(?,?,?,?,?)}",1);
        try {
            cs.setString(1,cnId);
...
//            java.math.BigDecimal unitId = new java.math.BigDecimal(pUnitId);
    ...
    /**
     * Container's getter for getLeaseNumber1.
     * @return getLeaseNumber1
     */
    public ViewObjectImpl getgetLeaseNumber1() {
        return (ViewObjectImpl)findViewObject("getLeaseNumber1");
    }
}
