...
//                              re.getAttribute("Uom") == null ? "" : re.getAttribute("Uom"));
            ...
            String ofFlg = Hcurr.getAttribute("OfferFlag")==null?"":Hcurr.getAttribute("OfferFlag").toString();
            if(ofFlg.equals("N")){
            //for updaing unit incharge and owner
            String owner = null;
            String untCatgry = null;
            String landlordVat = null;
            ...
            if(unitDtlsRoVo.getEstimatedRowCount()>0){
                ...
                String unitInChrg = r1.getAttribute("UnitIncharge")==null?"":r1.getAttribute("UnitIncharge").toString();
                    owner = r1.getAttribute("Owner")==null?"":r1.getAttribute("Owner").toString();
                    untCatgry = r1.getAttribute("UnitCategory")==null?"":r1.getAttribute("UnitCategory").toString();
                    landlordVat = r1.getAttribute("LandlordVat")==null?"":r1.getAttribute("LandlordVat").toString();
                Hcurr.setAttribute("UnitIncharge", unitInChrg);
                Hcurr.setAttribute("UnitOwner", owner);
                Hcurr.setAttribute("TaxCode", landlordVat);
                }
                utRSI.closeRowSetIterator();
            }
            if (untCatgry.equals(null) || untCatgry.equals("")){
                curr.setAttribute("Tra_UnitName",null);
                curr.setAttribute("UnitId",null);
                JSFUtils.addFacesErrorMessage("Unit Category is not defined in the unit master !!!");
            }
            
            if (owner.equals(null) || owner.equals("") ){
                curr.setAttribute("Tra_UnitName",null);
                curr.setAttribute("UnitId",null);
                JSFUtils.addFacesErrorMessage("Owner or primary is not defined in the unit master !!!");
            }else if (landlordVat.equals("")){
                curr.setAttribute("Tra_UnitName",null);
                curr.setAttribute("UnitId",null);
                JSFUtils.addFacesErrorMessage("Vat is not defined in landlord master or unit category is not defined in unit master !!!");
            }
            }//ofFlg condition end
...
//    }
    ...
    public void onClickAutoBooking(ActionEvent actionEvent) {
        ...
        String orgIdHdr=r.getAttribute("OrgId")==null?"null":r.getAttribute("OrgId").toString();
        if (!orgIdHdr.equalsIgnoreCase("300000021774129")){
...
//            JSFUtils.addFacesInformationMessage("Extension calculation executed");
    ...
    public void doCreateBKonClick(ActionEvent actionEvent) {
        // Add event code here...
        Row row = offerhdr.getCurrentRow();
        String org = row.getAttribute("OrgId") == null ? "" : row.getAttribute("OrgId").toString();
        if (!org.equalsIgnoreCase("300000021774129")){
        if (row.getAttribute("OfferFlag").equals("N") && row.getAttribute("FlowWith") == null){
            if(row.getAttribute("Attribute2") == null){
                row.setAttribute("Attribute2", "BK Created");
                onClickForBK();
        ...
        //                row.setAttribute("Attribute2", "BK Created");
                ...
                offerhdr.executeQuery();
                if (row.getAttribute("Attribute2") == null){
                    JSFUtils.addFacesInformationMessage("Booking is not created kindly check !!!");
                }else{
                JSFUtils.addFacesInformationMessage("Booking Created successfully !!!");
                }
            }else{
...
//        }
    ...
    public void doChangeStatusToDraft(ActionEvent actionEvent) {
        ...
        //calling pkg
        String flag = onDirectToDraft(ofHdrId,userId,funcId,reason);
        ADFUtils.findOperation("Commit").execute();
...
//                    perDisAmtBD = perDisAmtBD.setScale(2, BigDecimal.ROUND_UP);
    ...
    public void doPullSteRentPaBasedOnPl(ActionEvent actionEvent) {
        // Add event code here...
        Row row = offerhdr.getCurrentRow();
        String pullSetRentToTncy = ADFContext.getCurrent().getPageFlowScope().get("pullSetRentToTenancy")==null ? "" : ADFContext.getCurrent().getPageFlowScope().get("pullSetRentToTenancy").toString();
        ViewObject Currvo =
            ADFUtils.findIterator("OfferDetail_VO1Iterator").getViewObject();
        Row curr = Currvo.getCurrentRow();
        System.err.println("leadname="+row.getAttribute("Trans_LeadNumber"));
        //        if (row.getAttribute("OfferFlag").equals("R") || row.getAttribute("OfferFlag").equals("S")){
            ...
            if (plLinesVo.getEstimatedRowCount() > 0) {
                ...
                if(!pullSetRentToTncy.equalsIgnoreCase("")){
                    curr.setAttribute("Attribute5", basePrice);
                }
            }
    }
...
//                .divide(percntBD, 2);
    ...
    public void doChangeRenewal(ActionEvent actionEvent) {
        // Add event code here...
        Row row = offerhdr.getCurrentRow();
        String status = row.getAttribute("Status") == null ? "" : row.getAttribute("Status").toString();
        String ofHdrId = row.getAttribute("OfferHdrId") == null ? "0" : row.getAttribute("OfferHdrId").toString();        
        String userId =   ADFContext.getCurrent().getSessionScope().get("userId") == null ?
                            ...
                            ADFContext.getCurrent().getSessionScope().get("userId").toString();
        String funcId = row.getAttribute("FuncId") == null ? "0" : row.getAttribute("FuncId").toString();
        String reason = "RENEWAL CHANGE REQUEST";
        System.out.println("Status :"+status);
        //calling pkg
        if(status.equalsIgnoreCase("DRA")){
        String flag = onDirectToDraft(ofHdrId,userId,funcId,reason);
        ADFUtils.findOperation("Commit").execute();
        JSFUtils.addFacesInformationMessage("Renewal Change is done Successfully");
        }else{
            JSFUtils.addFacesErrorMessage("The offer must be on Draft !!!");
        }
            
    }
    public void doReopenOffer(ActionEvent actionEvent) {
        Row row = offerhdr.getCurrentRow();
        String status = row.getAttribute("Status") == null ? "" : row.getAttribute("Status").toString();
        String ofHdrId = row.getAttribute("OfferHdrId") == null ? "0" : row.getAttribute("OfferHdrId").toString();        
        String userId =   ADFContext.getCurrent().getSessionScope().get("userId") == null ?
                            ...
                            ADFContext.getCurrent().getSessionScope().get("userId").toString();
        String funcId = row.getAttribute("FuncId") == null ? "0" : row.getAttribute("FuncId").toString();
        String reason = "REOPEN OFFER REQUEST";
        System.out.println("Status :"+status);
        //calling pkg
        ...
        if(flag.equalsIgnoreCase("Success")){
        ADFUtils.findOperation("Commit").execute();
        JSFUtils.addFacesInformationMessage("Reopen of this offer done Successfully");
        }else{
            JSFUtils.addFacesErrorMessage("Kindly check the details !!!");
        }
        }else{
            JSFUtils.addFacesErrorMessage("The offer status must be expired !!!");
        }
    }
}
