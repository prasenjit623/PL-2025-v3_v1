...
public class BookingProcess {
    ...
    private RichInputComboboxListOfValues trans_CustIdArId;
    ...
    public void arReceiptCreatePayload(ActionEvent actionEvent) {
        ...
        try {
            ...
            if (receiptPayMode == null) {
                ...
                //landlord logic
                if(orgId.equals("300000021774129")){
                tnxTypeLndlrd = bookingVO.getCurrentRow().getAttribute("TnxTypeLandlord")==null ?"":bookingVO.getCurrentRow().getAttribute("TnxTypeLandlord").toString();
                cashInv = bookingVO.getCurrentRow().getAttribute("CashInvolved")==null ?"null":bookingVO.getCurrentRow().getAttribute("CashInvolved").toString();
                if(tnxTypeLndlrd.equalsIgnoreCase("Landlord")){
                lndlrdName = bookingVO.getCurrentRow().getAttribute("LandlordName")==null ?"null":bookingVO.getCurrentRow().getAttribute("LandlordName").toString();
                }
                }
...
//            }
            //calling pkg
            String resp = onDirectToDraft(bkId,userId,funcId,reason);
            ADFUtils.findOperation("Commit").execute();
    ...
    public String onDirectToDraft(String rcId,String userId,String funcId,String reason){
    ...
    public void setTrans_CustIdArId(RichInputComboboxListOfValues trans_CustIdArId) {
        this.trans_CustIdArId = trans_CustIdArId;
    }

    public RichInputComboboxListOfValues getTrans_CustIdArId() {
        return trans_CustIdArId;
    }

    public void doChangeCustomer(ActionEvent actionEvent) {
        // Add event code here...
        String arCust = trans_CustIdArId.getValue()==null?"0":trans_CustIdArId.getValue().toString();
        ViewObject bkCustvo=ADFUtils.findIterator("BookingCustomer_VO1Iterator").getViewObject();
        Row cusRow = bkCustvo.getCurrentRow();
        String bkCustId = cusRow.getAttribute("BookCustId") == null ? "0" : cusRow.getAttribute("BookCustId").toString(); 
        String custIdAr = cusRow.getAttribute("CustIdAr") == null ? "0" : cusRow.getAttribute("CustIdAr").toString();
        String userId =   ADFContext.getCurrent().getSessionScope().get("userId") == null ?
                            null :
                            ADFContext.getCurrent().getSessionScope().get("userId").toString();
        String funcId = "7";
        String reason = custIdAr+":CUSTOMER CHANGE REQUEST";
        //calling pkg
        if(!arCust.equals("0") && !bkCustId.equals("0")){
        //customer change in same pkg
            String resp = onDirectToDraft(bkCustId,userId,funcId,reason);
                if (resp.equalsIgnoreCase("Success")){
                ADFUtils.findOperation("Commit").execute();
                JSFUtils.addFacesInformationMessage("Customer Change is done Successfully");
                }else{
                    JSFUtils.addFacesErrorMessage("Customer Changes is not done, kindly check !!!");
                }
        }else{
            JSFUtils.addFacesErrorMessage("Customer Changes is not done !!!");
        }
    }
    
    public void doChngTnxPaymentTo(ActionEvent actionEvent) {
        ViewObject bkVo=ADFUtils.findIterator("BookingHeader_VO1Iterator").getViewObject();
        Row bkRow = bkVo.getCurrentRow();
        String bkId = bkRow.getAttribute("BookingId1") == null ? "0" : bkRow.getAttribute("BookingId1").toString();
        String bkPaymtTo = bkRow.getAttribute("CashInvolved") == null ? "0" : bkRow.getAttribute("CashInvolved").toString();
        String userId =   ADFContext.getCurrent().getSessionScope().get("userId") == null ?
                            null :
                            ADFContext.getCurrent().getSessionScope().get("userId").toString();
        String resp = doChangeTnxPaymentTo(bkId,userId,"7",bkPaymtTo,"TNX_PAYMENT_TO");
        if (resp.equalsIgnoreCase("Success")){
        ADFUtils.findOperation("Commit").execute();
        JSFUtils.addFacesInformationMessage("Payment To Changes is done Successfully");
        }else{
            JSFUtils.addFacesErrorMessage("Payment To Changes is not done, kindly check !!!");
        }
    }
    //caliing package for change of tnx payment to
    public String doChangeTnxPaymentTo(String refId,String userId,String funcId,String changeData,String reason){
        
              OperationBinding op=ADFUtils.findOperation("changeTnxData");
              op.getParamsMap().put("refId",refId);
              op.getParamsMap().put("userId",userId);
              op.getParamsMap().put("funcId",funcId);
              op.getParamsMap().put("changeData",changeData);
              op.getParamsMap().put("reason",reason);
              String flag=op.execute().toString();
              JSFUtils.addFacesInformationMessage(flag);
            return flag;
