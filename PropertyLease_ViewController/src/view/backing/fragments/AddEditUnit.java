...
//          JSFUtils.addFacesInformationMessage(newvalues.toString()+""+oldvalues.toString());
    ...
    public void onSelectValidateBu(ValueChangeEvent valueChangeEvent) {
        valueChangeEvent.getComponent().processUpdates(FacesContext.getCurrentInstance());
        String v = valueChangeEvent.getNewValue()==null?"":valueChangeEvent.getNewValue().toString();
        ViewObject unitDtlRoVo = ADFUtils.findIterator("UnitDtls_RoVo1Iterator").getViewObject();
        ViewObject unitVo = ADFUtils.findIterator("PropertyUnits_VO1Iterator").getViewObject();
        Row unitCurrRow = unitVo.getCurrentRow();
        ViewCriteria vC = unitDtlRoVo.createViewCriteria();
        ViewCriteriaRow vCRow = vC.createViewCriteriaRow();
        vCRow.setAttribute("UnitId", unitCurrRow.getAttribute("UnitId"));
        vCRow.setAttribute("OrgId", "300000021774129");
        vC.addRow(vCRow);
        unitDtlRoVo.applyViewCriteria(vC);
        unitDtlRoVo.executeQuery();
        if (unitDtlRoVo.getEstimatedRowCount() == 0) {
            soc24.setValue(null);
            soc25.setValue(null);
            it34.setValue(null);
            JSFUtils.addFacesErrorMessage("This is only applicable for Alfa Smart BU !!!");           
        }else{
            if(v.equalsIgnoreCase("Investment")){
                soc25.setValue(null);
            }
        }
        AdfFacesContext.getCurrentInstance().addPartialTarget(soc24);
        AdfFacesContext.getCurrentInstance().addPartialTarget(soc25);
        AdfFacesContext.getCurrentInstance().addPartialTarget(it34);
    }
    public void onSelectUnitLoc(ValueChangeEvent valueChangeEvent) {
        valueChangeEvent.getComponent().processUpdates(FacesContext.getCurrentInstance());
        String v = valueChangeEvent.getNewValue()==null?"":valueChangeEvent.getNewValue().toString();
        ViewObject unitDtlRoVo = ADFUtils.findIterator("UnitDtls_RoVo1Iterator").getViewObject();
        ViewObject unitVo = ADFUtils.findIterator("PropertyUnits_VO1Iterator").getViewObject();
        ...
        if (unitDtlRoVo.getEstimatedRowCount() == 0) {
            soc26.setValue(null);
            JSFUtils.addFacesErrorMessage("This is only applicable for Alfa Smart BU !!!");
            AdfFacesContext.getCurrentInstance().addPartialTarget(soc26);
        }
    }
    //wo
    public void onSlctLandlrdCustTagWo(ValueChangeEvent valueChangeEvent) {
        valueChangeEvent.getComponent().processUpdates(FacesContext.getCurrentInstance());
        String v = valueChangeEvent.getNewValue()==null?"":valueChangeEvent.getNewValue().toString();
        ViewObject unitDtlRoVo = ADFUtils.findIterator("UnitDtls_RoVo1Iterator").getViewObject();
        ViewObject unitVo = ADFUtils.findIterator("PropertyUnits_VO1Iterator").getViewObject();
        ...
        if (unitDtlRoVo.getEstimatedRowCount() == 0) {
            soc27.setValue(null);
            JSFUtils.addFacesErrorMessage("This is only applicable for Alfa Smart BU !!!");
            AdfFacesContext.getCurrentInstance().addPartialTarget(soc27);
        }
    }
