...
public class AddEditWoInvHdrBb {
    ...
    public void doGenerateInvoice(ActionEvent actionEvent) throws IOException {
        ...
        try {
            ...
            //initial commit incase not click save button and click generate invoice
            ADFUtils.findOperation("Commit").execute();
            Row row = woInvHdrVo.getCurrentRow();
            ...
            String rcCost = row.getAttribute("ResourceCost")==null ? "":row.getAttribute("ResourceCost").toString();
            String mcCost = row.getAttribute("MaterialCost")==null ? "":row.getAttribute("MaterialCost").toString();
            String stagRcCost = row.getAttribute("StagResourceCost")==null ? "":row.getAttribute("StagResourceCost").toString();
            String stagMcCost = row.getAttribute("StagMaterialCost")==null ? "":row.getAttribute("StagMaterialCost").toString();
            String custNo = row.getAttribute("Customernumber")==null ? "":row.getAttribute("Customernumber").toString();
            String woNoHdr = row.getAttribute("WorkOrder")==null ? "":row.getAttribute("WorkOrder").toString();
            String invStsHdr = row.getAttribute("InvStatus")==null ? "":row.getAttribute("InvStatus").toString();
            String clsdDateMnYrHdr = (row.getAttribute("ClosedDate")==null ? "":row.getAttribute("ClosedDate").toString()).substring(0, 7);
            ViewObject srchWoInvRoVo = ADFUtils.findIterator("SearchWoInv_RoVo2Iterator").getViewObject();  
            ViewCriteria srchVc=srchWoInvRoVo.createViewCriteria();
            ViewCriteriaRow srchVcr=srchVc.createViewCriteriaRow();
            srchVcr.setAttribute("WorkOrder",woNoHdr);
            srchVcr.setAttribute("InvStatus","INVOICED");
            srchVcr.setAttribute("ClosedDate","like '%"+clsdDateMnYrHdr+"%'");
            srchVc.addRow(srchVcr);
            srchWoInvRoVo.applyViewCriteria(srchVc);
            srchWoInvRoVo.executeQuery();
            if(srchWoInvRoVo.getEstimatedRowCount()==0){ //already invoiced for same work order and period validation
            srchWoInvRoVo.applyViewCriteria(null);
            srchWoInvRoVo.executeQuery();
            if(rcCost.equalsIgnoreCase(stagRcCost) && mcCost.equalsIgnoreCase(stagMcCost) && !custNo.equals("")){
        ...
        //                while (invoiceRS.hasNext()) {
                Row currRow = woInvHdrVo.getCurrentRow();
        ...
        //                    }
                    ...
                    row.setAttribute("TrxDate", ruleStartDate);
        ...
        //                    Object unitSellingPrice = currRow.getAttribute("MaterialCost");
                    ...
                    Object amount = "0";
                    Object chrgType = "";
                ViewObject getWoInvDtlArRoVo = ADFUtils.findIterator("GetWoInvDtlAr_RoVo1Iterator").getViewObject();  
                ViewCriteria vc=getWoInvDtlArRoVo.createViewCriteria();
                ViewCriteriaRow vcr=vc.createViewCriteriaRow();
                vcr.setAttribute("WoInvHdrId",woInvHdrId);
                vc.addRow(vcr);
                getWoInvDtlArRoVo.applyViewCriteria(vc);
                getWoInvDtlArRoVo.executeQuery();
                RowSetIterator rsiAR = getWoInvDtlArRoVo.createRowSetIterator(null);
                int i=1;
                while(rsiAR.hasNext()) {  
                    System.out.println("iterator no i :"+i); 
                    JSFUtils.addFacesInformationMessage("Iterator ::: "+i);
                    Row rowArLine = rsiAR.next();
                chrgType = rowArLine.getAttribute("Charges");
                String lineAmt = rowArLine.getAttribute("Amount")==null ? "0" : rowArLine.getAttribute("Amount").toString();                    
                if(lineAmt.equalsIgnoreCase("0")){
                    //no additional line(4th line) for invoice---skip 4th line
                }else{
        ...
        //                }
                ...
                System.out.println("Tax code WO inv :"+taxCode);
                    ...
                    if (validationFlag == "Y") {
                        ...
                        Request request =
                            new Request.Builder().url("https://almlbprd.miskprops.com/Al-MiskIntegrations/webresources/invoicecreation/v1").post(body).addHeader("Content-Type",
        ...
        //                        AdfFacesContext.getCurrentInstance().addPartialTarget(generate_invoice_cb15);
            ...
            //            }
                ...
                row.setAttribute("InvStatus", "Error");              
                if(custNo.equals("")){
                    row.setAttribute("Attribute2", "Customer is not there in this work order");
                    JSFUtils.addFacesInformationMessage("Customer is not there in this work order !!!");
                }else{
                    row.setAttribute("Attribute2", "Check the resource and material amount");
                    JSFUtils.addFacesInformationMessage("Check the resource and material amount !!!");
                }
                ADFUtils.findOperation("Commit").execute();
            }
            }else{ //already invoiced validation
                srchWoInvRoVo.applyViewCriteria(null);
                srchWoInvRoVo.executeQuery();
                row.setAttribute("InvStatus", "Error");
                row.setAttribute("Attribute2", "Already invoiced for same work order and same period of month and year");
                ADFUtils.findOperation("Commit").execute();
                JSFUtils.addFacesErrorMessage("Already invoiced for same work order and same period of month and year !!!");
            }
    ...
    //only for userId 100
    public void doGenerateInvoiceOnly(ActionEvent actionEvent) throws IOException {
        // Add event code here...
        try {
        ...
        //                    Object unitSellingPrice = currRow.getAttribute("MaterialCost");
                ...
                String lineAmt = rowArLine.getAttribute("Amount")==null ? "0" : rowArLine.getAttribute("Amount").toString();                    
                if(lineAmt.equalsIgnoreCase("0")){
                    //no line for interface if line amount is zero 
                }else{
                    description=chrgType;
                amount = rowArLine.getAttribute("Amount")==null ? "0" : rowArLine.getAttribute("Amount");
        //                if (billableStatus.equalsIgnoreCase("Under Contract")){
...
//                            otherChargeNumber.substring(1, 8).toString(); //OC-3017
    ...
    public void doSelectTaxCode(ValueChangeEvent valueChangeEvent) {
        ...
        String totAmt = r.getAttribute("TotalAmount")==null?"0":r.getAttribute("TotalAmount").toString();
        String taxCode = valueChangeEvent.getNewValue()==null ? "0" : valueChangeEvent.getNewValue().toString();
        Row row = woInvHdrVo.getCurrentRow();
        String totInvCost = row.getAttribute("TotalInvCost")==null ? "0" : row.getAttribute("TotalInvCost").toString();
        String totWoInvAmt = row.getAttribute("TotalWoInvAmount")==null ? "0" : row.getAttribute("TotalWoInvAmount").toString();
        String chargeTypLine = r.getAttribute("Charges")==null ? "" : r.getAttribute("Charges").toString();
        if(valueChangeEvent.getNewValue()!=null){
            taxCbd = new BigDecimal(taxCode);
            if(chargeTypLine.equalsIgnoreCase("WO INVOICE VARIANCE AMOUNT")){
                totlAmtBD = new BigDecimal(totAmt);
                taxamtBD=(totlAmtBD.multiply(taxCbd)).divide((taxCbd.add(percntBD)),2, RoundingMode.HALF_UP);//tax amount in arithmetic process
                amtBD = totlAmtBD.subtract(taxamtBD);
                r.setAttribute("Amount", amtBD);
        ...
        BigDecimal totlInvArAmtAddtnBD = new BigDecimal(0);
        ...
        while (rs.hasNext()) {
            ...
            String chargeTyp = rowAr.getAttribute("Charges")==null ? "" : rowAr.getAttribute("Charges").toString();
            System.out.println("Total amt on tax line wise ::"+totlAmt);
            if(!chargeTyp.equalsIgnoreCase("WO INVOICE VARIANCE AMOUNT")){
            BigDecimal invArAmtBD = new BigDecimal(totlAmt);
            totlInvArAmtBD = totlInvArAmtBD.add(invArAmtBD);
            }
            //for overall cal only
            if(chargeTyp.equalsIgnoreCase("WO INVOICE VARIANCE AMOUNT")){
                 woLineTaxCode = rowAr.getAttribute("TaxCode")==null ? "0" : rowAr.getAttribute("TaxCode").toString();
            }        
            //
            BigDecimal invArAmtAddtnBD = new BigDecimal(totlAmt);
            totlInvArAmtAddtnBD = totlInvArAmtAddtnBD.add(invArAmtAddtnBD);
        }
        rs.closeRowSetIterator();
        woInvHdrVo.getCurrentRow().setAttribute("TotalInvCost", totlInvArAmtBD);
//        if(!totWoInvAmt.equalsIgnoreCase("0")){
//        woInvHdrVo.getCurrentRow().setAttribute("TotalWoInvAmount", totlInvArAmtAddtnBD);
//        }
                //Recalculate based on tot wo inv amt
        if(chargeTypLine.equalsIgnoreCase("WO INVOICE VARIANCE AMOUNT")){
            woLineTaxCode = valueChangeEvent.getNewValue().toString();
        }
        System.err.println("woLineTaxCode ::"+woLineTaxCode);
        onRecalcOnWoTotInvAmt(woLineTaxCode);
    }
    ...
    public void onSelectWoNo(ValueChangeEvent valueChangeEvent) {
        ...
        BigDecimal matMarkupCostDB = new BigDecimal(0);
        BigDecimal resMarkupCostDB = new BigDecimal(0);
        ...
        String matMarkupCost = "0";
        String resMarkupCost = "0";
        String createResMrkup = "NO";
        String propOrgId = "0";
        Row row = woInvHdrVo.getCurrentRow();
        ...
        if(getWoInfoRoVo.getEstimatedRowCount()>0){
            ...
            createResMrkup = getWoInfoRoVo.first().getAttribute("CreateResourceMarkup")==null ?"NO":getWoInfoRoVo.first().getAttribute("CreateResourceMarkup").toString();
            propOrgId = getWoInfoRoVo.first().getAttribute("PropertyOrgId")==null ?"0":getWoInfoRoVo.first().getAttribute("PropertyOrgId").toString();
        }
//        System.out.println("mCost %%"+materialCost+" rCost %%"+resourceCost);
        ...
        if(valueChangeEvent.getNewValue()!=null){
            ...
            matMarkupCostDB = mtrlCostBD.multiply(markupPercentDB);
            matMarkupCostDB = matMarkupCostDB.setScale(2, BigDecimal.ROUND_HALF_EVEN);
            //LOGIC : wo create res markup yes in unit master //updated on 10-Jn-25
            if(createResMrkup.equalsIgnoreCase("YES")){
            resMarkupCostDB = rscCostBD.multiply(markupPercentDB);
            resMarkupCostDB = resMarkupCostDB.setScale(2, BigDecimal.ROUND_HALF_EVEN);
            }
            totlInvCostDB = ((mtrlCostBD.add(rscCostBD)).add(matMarkupCostDB)).add(resMarkupCostDB);
        //            doCreateArLinesCalPkg(woInvHdrId,materialCost,resourceCost,userName);
            row.setAttribute("TotalInvCost", totlInvCostDB);
            matMarkupCost = matMarkupCostDB.toString();
            resMarkupCost = resMarkupCostDB.toString();
//            System.out.println("woInvHdrId::"+woInvHdrId+" materialCost ::"+materialCost+" resourceCost ::"+resourceCost+" matMarkupCost ::"+matMarkupCost+" resMarkupCost ::"+resMarkupCost+" userName ::"+userName);
            //do insert
            doInsrtWoInvArLines(woInvHdrId,materialCost,resourceCost,matMarkupCost,resMarkupCost,userName,createResMrkup,propOrgId);
        }       
    }
    
    public void doInsrtWoInvArLines(String woInvHdrId,String materialCost,String resourceCost,String matMarkupCost,String resMarkupCost,String userName,
                                    String createResMrkup,String propOrgId) {
        RowSetIterator rs = woInvDtlArVo.createRowSetIterator(null);
        while (rs.hasNext()) {
            Row r = rs.next();
            r.remove();
    ...
    //        rwM.setAttribute("LastUpdateDate", "");
        ...
        rwMup.setAttribute("Charges", "MATERIAL MARKUP");
        rwMup.setAttribute("TaxCode", "0");
        rwMup.setAttribute("TaxAmount", "0");
        rwMup.setAttribute("Amount", matMarkupCost);
        rwMup.setAttribute("TotalAmount", matMarkupCost);
        ...
        //Inserrt ADJUSTMENT line 4
        Row rwAdj = woInvDtlArVo.createRow();
        rwAdj.setAttribute("WoInvHdrId", woInvHdrId);
        rwAdj.setAttribute("Charges", "WO INVOICE VARIANCE AMOUNT");
        rwAdj.setAttribute("TaxCode", "0");
        rwAdj.setAttribute("TaxAmount", "0");
        rwAdj.setAttribute("Amount", "0");
        rwAdj.setAttribute("TotalAmount", "0");
        rwAdj.setAttribute("CreatedBy", userName);
        //        rwAdj.setAttribute("CreationDate", "");
        rwAdj.setAttribute("LastUpdatedBy", userName);
        //        rwAdj.setAttribute("LastUpdateDate", "");
        woInvDtlArVo.insertRow(rwAdj);
        //LOGIC : wo create res markup yes in unit master //updated on 10-Jn-25
        if(createResMrkup.equalsIgnoreCase("YES")){
        //Inserrt Resource Markup line 5
        Row rwRMup = woInvDtlArVo.createRow();
        rwRMup.setAttribute("WoInvHdrId", woInvHdrId);
        rwRMup.setAttribute("Charges", "RESOURCE MARKUP");
        rwRMup.setAttribute("TaxCode", "0");
        rwRMup.setAttribute("TaxAmount", "0");
        rwRMup.setAttribute("Amount", resMarkupCost);
        rwRMup.setAttribute("TotalAmount", resMarkupCost);
        rwRMup.setAttribute("CreatedBy", userName);
        //        rwRMup.setAttribute("CreationDate", "");
        rwRMup.setAttribute("LastUpdatedBy", userName);
        //        rwRMup.setAttribute("LastUpdateDate", "");
        woInvDtlArVo.insertRow(rwRMup);
        }
    }
    ...
    public void onEntrTotWoInvAmt(ValueChangeEvent valueChangeEvent) {
        String totWoInvAmt = valueChangeEvent.getNewValue()==null ? "0" : valueChangeEvent.getNewValue().toString();
        Row row = woInvHdrVo.getCurrentRow();
        String totInvCost = row.getAttribute("TotalInvCost")==null ? "0" : row.getAttribute("TotalInvCost").toString();
        BigDecimal totWoInvAmtBD = new BigDecimal(totWoInvAmt);//with additional
        BigDecimal totInvCostBD = new BigDecimal(totInvCost);//without additional
        BigDecimal addtnalAmt = totWoInvAmtBD.subtract(totInvCostBD);   
        BigDecimal taxCbd = new BigDecimal(0);
        BigDecimal percntBD = new BigDecimal(100);
        BigDecimal AmtBD = new BigDecimal(0);
        BigDecimal taxamtBD = new BigDecimal(0);
        if(addtnalAmt.compareTo(BigDecimal.ZERO) > 0){
        RowSetIterator rs = woInvDtlArVo.createRowSetIterator(null);
        while (rs.hasNext()) {
            Row rowAr = rs.next();   
            String chrges = rowAr.getAttribute("Charges")==null ? "" : rowAr.getAttribute("Charges").toString();
            if(chrges.equalsIgnoreCase("WO INVOICE VARIANCE AMOUNT")){
            String taxCode = rowAr.getAttribute("TaxCode")==null?"0":rowAr.getAttribute("TaxCode").toString();            
            taxCbd = new BigDecimal(taxCode);
            taxamtBD=(addtnalAmt.multiply(taxCbd)).divide((taxCbd.add(percntBD)),2, RoundingMode.HALF_UP);//tax amount in arithmetic process
            System.out.println("taxamtBD===="+taxamtBD);
            AmtBD = addtnalAmt.subtract(taxamtBD);           
                rowAr.setAttribute("Amount", AmtBD.toString());
                rowAr.setAttribute("TaxAmount", taxamtBD);
                rowAr.setAttribute("TotalAmount", addtnalAmt.toString());
            }
        }
        rs.closeRowSetIterator();
        }else{
            ...
            while (rs.hasNext()) {
                ...
                String chrges = rowAr.getAttribute("Charges")==null ? "" : rowAr.getAttribute("Charges").toString();
                if(chrges.equalsIgnoreCase("WO INVOICE VARIANCE AMOUNT")){
                    rowAr.setAttribute("Amount", 0);
                    rowAr.setAttribute("TaxCode", 0);
                    rowAr.setAttribute("TaxAmount", 0);
                    rowAr.setAttribute("TotalAmount", 0);
                }
            }
            rs.closeRowSetIterator();
            it41.setValue(null);
            AdfFacesContext.getCurrentInstance().addPartialTarget(it41);
            JSFUtils.addFacesErrorMessage("The entered Total WO Inv Amount can't be less than or equal to normal Total cost !!!");
        }
    ...
    public void onRecalcOnWoTotInvAmt(String taxCod) {
        Row row = woInvHdrVo.getCurrentRow();
        String totWoInvAmt = row.getAttribute("TotalWoInvAmount")==null?"0":row.getAttribute("TotalWoInvAmount").toString();
        String totInvCost = row.getAttribute("TotalInvCost")==null ? "0" : row.getAttribute("TotalInvCost").toString();
        BigDecimal totWoInvAmtBD = new BigDecimal(totWoInvAmt);//with additional
        ...
        while (rs.hasNext()) {
            ...
            if(chrges.equalsIgnoreCase("WO INVOICE VARIANCE AMOUNT")){
    //            String taxCode = rowAr.getAttribute("TaxCode")==null?"0":rowAr.getAttribute("TaxCode").toString();
            String taxCode = taxCod;
            System.err.println("taxCode in main -"+taxCode);
            System.err.println("Carried taxCode in main -"+taxCod);
            taxCbd = new BigDecimal(taxCode);
            taxamtBD=(addtnalAmt.multiply(taxCbd)).divide((taxCbd.add(percntBD)),2, RoundingMode.HALF_UP);//tax amount in arithmetic process
            System.out.println("taxamtBD main===="+taxamtBD);
            AmtBD = addtnalAmt.subtract(taxamtBD);           
                rowAr.setAttribute("Amount", AmtBD.toString());
                rowAr.setAttribute("TaxAmount", taxamtBD);
                rowAr.setAttribute("TotalAmount", addtnalAmt.toString());
    ...
    public void doBulkSyncInvoice(ActionEvent actionEvent) {
        Row row = woInvHdrVo.getCurrentRow();
        String woInvHdrId = row.getAttribute("WoInvHdrId")==null ? "0" : row.getAttribute("WoInvHdrId").toString();
//        System.out.println("%%%woInvHdrId%%%"+woInvHdrId);
        String userName =
            ADFContext.getCurrent().getSessionScope().get("userName") == null ?
            null :
            ADFContext.getCurrent().getSessionScope().get("userName").toString();
        OperationBinding op=ADFUtils.findOperation("callBulkSyncInv");
        op.getParamsMap().put("p_woInvHdrId",woInvHdrId);
        op.getParamsMap().put("p_userName",userName);
        String flag=op.execute().toString();
        woInvHdrVo.executeQuery();
        AdfFacesContext.getCurrentInstance().addPartialTarget(it38);
        JSFUtils.addFacesInformationMessage(flag);
